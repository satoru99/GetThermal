...
try:
    if platform.system() == 'Darwin':
        ...
        #libuvc = "libuvc.so"
        libuvc = "libgetthermal.so"
    else:
        libuvc = "libuvc"
    if os.path.exists(os.path.join(here, libuvc)):
        libuvc = os.path.join(here, libuvc)
    print(libuvc)
...
class LeptonVariation(object):
    def __init__(self, ctx, dev, devh):
        libuvc.leptonvariation_create.restype = c_void_p
        self.v = libuvc.leptonvariation_create(ctx, dev, devh)

    def __del__(self):
        libuvc.leptonvariation_delete.argtypes = [c_void_p]
        libuvc.leptonvariation_delete(self.v)

    def performFfc(self):
        libuvc.leptonvariation_performFfc.argtypes = [c_void_p]
        libuvc.leptonvariation_performFfc(self.v)

    def getauxtempC(self):
        libuvc.leptonvariation_getauxtempC.restype = c_int
        libuvc.leptonvariation_getauxtempC.argtypes = [c_void_p, POINTER(c_float)]
        auxTemperature = c_float()
        result = libuvc.leptonvariation_getauxtempC(self.v, pointer(auxTemperature))
        return auxTemperature.value

    def getauxtempK(self):
